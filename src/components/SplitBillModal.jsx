...
import { X, Users, Check, CreditCard, PieChart } from 'lucide-react';
...
const SplitBillModal = ({ tx, onBack, onConfirm }) => {
  ...
  const splitAmount = Math.round(txAmount / (selectedPlayers.length + 1));
  ...
  const handleConfirmSplit = () => {
    setStep('success');
    setTimeout(() => {
      onConfirm(tx, selectedPlayers, splitAmount);
    }, 1500);
  };
      ...
      case 'select_members':
        return (
          <>
            ...
            <div className="glass-card" style={{ background: 'rgba(255,255,255,0.05)', border: '1px dashed var(--glass-border)' }}>
              <p className="muted" style={{ fontSize: '0.8rem' }}>Tìm bạn bè khác...</p>
            </div>
      ...
      case 'calculate':
        return (
          <div style={{ padding: '10px 0' }}>
            <div className="glass-card neon-border" style={{ textAlign: 'center', marginBottom: '30px' }}>
              <PieChart size={32} color="var(--primary)" style={{ margin: '0 auto 10px' }} />
              <p className="muted">Tổng cộng</p>
              <h2 style={{ fontSize: '2rem' }}>{txAmount.toLocaleString()}đ</h2>
            </div>

            <div className="section-title"><h3>Kết quả chia đều ({selectedPlayers.length + 1} người)</h3></div>
            <div className="split-list">
              <div className="tx-item" style={{ background: 'rgba(195, 255, 0, 0.05)', border: '1px solid rgba(195, 255, 0, 0.2)' }}>
                ...
                <div className="tx-info"><h4>Bạn</h4><p className="muted">Người trả tiền</p></div>
                <div style={{ fontWeight: 'bold' }}>{splitAmount.toLocaleString()}đ</div>
              </div>
              {selectedPlayers.map(p => (
                <div key={p.id} className="tx-item">
                  ...
                  <div className="tx-info"><h4>{p.name}</h4><p className="muted">Chưa thu</p></div>
                  <div style={{ fontWeight: 'bold' }}>{splitAmount.toLocaleString()}đ</div>
                </div>
              ))}
            </div>
      ...
      case 'success':
        return (
          <div style={{ textAlign: 'center', padding: '60px 0' }}>
            ...
            <p className="muted">Chúng tôi sẽ thông báo cho các thành viên</p>
          </div>
        );
      default: return null;
    }
  ...
  return (
    <div className="booking-overlay active">
      <div className="booking-header" style={{ padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h2 style={{ margin: 0 }}>Chia tiền sân</h2>
        {step !== 'success' ? (
      ...
      <div className="screen-content" style={{ padding: '0 20px 40px', flex: 1, overflowY: 'auto' }}>
        <div className="glass-card" style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <div>
            ...
            <p className="muted" style={{ fontSize: '0.8rem' }}>{tx.date} • {tx.amount}</p>
          </div>
        </div>
        {renderContent()}
      </div>
